          ...
          {SECTIONS.slice(1).map((s) => (
            <li key={s.id}>
              <a
                ...
                className={`rounded-chip px-3 py-2 font-mono text-[11px] tracking-[0.18em] uppercase transition-colors duration-200 ${
                  active === s.id
                    ? 'text-amber-text'
                    : 'text-dim hover:text-ink'
                }`}
              >
                <span className="mr-1.5 opacity-60">{s.numeral}</span>
                {s.title}
              </a>
            </li>
          ))}
